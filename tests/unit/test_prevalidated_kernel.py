...
def test_execute_rejects_missing_prevalidation():
    ...
    assert client.post("/execute", json=body).status_code == 422
...
def test_execute_rejects_denied_policy_or_authz():
    ...
    assert client.post("/execute", json=body).status_code == 403
...
def test_memory_rejects_missing_tenant():
    body = _base_body()
    del body["tenant_id"]
    assert client.post("/memory", json=body).status_code == 422
...
def test_idempotency_key_returns_same_execution_response():
    body = _base_body("t-ido")
    headers = {"X-Idempotency-Key": "idem-123"}
    first = client.post("/execute", json=body, headers=headers)
    second = client.post("/execute", json=body, headers=headers)
