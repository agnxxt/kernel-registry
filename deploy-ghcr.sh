...
resolve_public_host() {
  ...
  [ -n "$PUBLIC_HOST" ] || fail "Could not detect public IP. Rerun with PUBLIC_HOST=your.domain.com bash deploy-ghcr.sh"
}
...
NEXT_PUBLIC_API_URL=http://$PUBLIC_HOST:8000

KERNEL_BACKEND_IMAGE=${KERNEL_BACKEND_IMAGE:-ghcr.io/AGenNext/kernel-registry-backend:latest}
KERNEL_UI_IMAGE=${KERNEL_UI_IMAGE:-ghcr.io/AGenNext/kernel-registry-ui:latest}
...
start_app() {
  log "Pulling and starting GHCR images"
...
Useful commands:
  cd $APP_DIR && sudo docker compose -f compose.ghcr.yaml ps
  cd $APP_DIR && sudo docker compose -f compose.ghcr.yaml logs -f

One-line GHCR install:
  curl -fsSL https://raw.githubusercontent.com/AGenNext/kernel-registry/main/deploy-ghcr.sh | bash
EOF
}
